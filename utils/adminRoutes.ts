...
export const resolveAdminRoute = (pathname: string, isAdmin: boolean): AdminRouteResolution => {
    ...
    if (pathname.startsWith('/admin/me/albums') || pathname.startsWith('/me/albums')) return { tab: 'photos' };
    if (pathname.startsWith('/admin/me/analytics') || pathname.startsWith('/me/analytics')) return { tab: 'stats' };
    if (pathname.startsWith('/admin/me/uploads') || pathname.startsWith('/me/uploads')) return { tab: 'me_uploads' };
    if (pathname.startsWith('/admin/me/profile') || pathname.startsWith('/me/profile')) return { tab: 'settings' };
